...
module.exports = {
  ...
  treeForPublic: function(app) {
    ...
    var config = Object.keys(DEFAULT_OPTIONS).reduce(function(acc, key) {
      acc[key] = clientConfig[key] || DEFAULT_OPTIONS[key];
      return acc;
    }, {});
    ...
    return new BroccoliMergeTrees([
        generate(null, {
          ...
          template: '{{data}}',
            values: {
              data: Object.keys(config).map(function(key) {
                return key + '=' + config[key];
              }).join('&')
            }
        }),
      new Funnel(path.join(this.project.root, 'public'), {
        srcDir: '/',
        destDir: '/',
